      ...
      },
      {
        ...
        rules: [
          ...
          },
          {
            ...
            // could never revoke. Mostly scoped to deployments that set
            // superseded_cert_revoke_after_sec — with no delay the revocation
            // happens inside the renewal and a failure there is a CRL failure,
            // counted by the alert above instead. One exception, and it is the
            // one a deployment without the feature will hit: the sweep, every
            // renewal and every subject revocation read the pending list
            // whatever the setting says, so a store that cannot serve that key
            // fires this anywhere. Counter resets on restart, so alert on
