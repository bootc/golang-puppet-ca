...
var _ = Describe("Collector", func() {
	...
	// All four chain counters must be exported on a CA that has never configured
	// crl_chain_file, and must read 0 there.
	//
	// Two of them were only ever gathered after the feature was switched on, so
	// a MustNewConstMetric line moved inside a `if c.CRLChainFile != ""` guard
	// -- which is how the *gauge* beside them is correctly written -- would have
	// left the suite green. That is not a cosmetic difference: an absent counter
	// makes `increase(...)` return nothing rather than 0, so the shipped alerts
	// would go from quiet-and-correct to quiet-and-blind on every CA in the
	// fleet that does not use the feature, and nothing distinguishes the two.
	// The mixin has the matching guard from the alert side ("a CA not publishing
	// an upstream chain is silent"); this is the exporter side of it.
	It("exports every chain counter at zero on a CA with no crl_chain_file", func() {
		Expect(myCA.CRLChainFile).To(BeEmpty(), "this spec must run unconfigured")

		g := gather(metrics.NewCollector(myCA))
		for _, name := range []string{
			"puppetca_crl_chain_refresh_failures_total",
			"puppetca_crl_chain_discarded_total",
			"puppetca_crl_chain_regressed_total",
			"puppetca_crl_chain_removed_total",
		} {
			Expect(g[name]).NotTo(BeNil(), "%s must be exported even when unconfigured", name)
			Expect(counterValue(g.findByLabels(name, nil))).To(Equal(0.0),
				"%s must read 0 on a CA that never enabled the feature", name)
		}
	})
	...
	It("exports the removal counter by name, and at its value", func() {
		...
		g = gather(metrics.NewCollector(myCA))
		// One, for one ancestor, and asserting the value rather than a threshold
		// is what surfaced the bug this pins the fix for. The pass that performs
		// a removal reads the file twice -- once to decide a rewrite is needed,
		// once inside reissueCRLLocked to build what it publishes -- and the
		// ancestor is still in the published chain for both. Counting on both
		// made one lost ancestor read as 2, so only the publishing read counts;
		// see upstreamPass in internal/ca/crlchainfile.go. A `>= 1` assertion
		// here would have passed throughout and pinned nothing.
		Expect(counterValue(g.findByLabels("puppetca_crl_chain_removed_total", nil))).
			To(Equal(1.0),
				"once for one ancestor: only the publishing read counts")

		// And bounded: once the chain is published without it, the ancestor is
		// no longer in `published`, so nothing detects it again. Without this
		// the counter could climb on every pass for ever and the assertion above
		// would still hold on the first gather.
		Expect(myCA.RefreshCRLChainFile(ctx)).Error().NotTo(HaveOccurred())
		g = gather(metrics.NewCollector(myCA))
		Expect(counterValue(g.findByLabels("puppetca_crl_chain_removed_total", nil))).
			To(Equal(1.0), "a removal is counted while it happens, not on every pass after it")
		Expect(counterValue(g.findByLabels("puppetca_crl_chain_discarded_total", nil))).
