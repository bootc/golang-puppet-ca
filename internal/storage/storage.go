...
// countUnparseableInventoryLines reports how many non-blank lines of a blob
// inventory parseInventoryBlob discarded. The blob MAC covers them; the entry
// count does not describe them.
func countUnparseableInventoryLines(data []byte, parsed int) int {
	var nonBlank int
	for _, line := range strings.Split(string(data), "\n") {
		if strings.TrimSpace(line) != "" {
			nonBlank++
...
func (s *StorageService) InventoryIntegrityReport(ctx context.Context) (InventoryIntegrityReport, error) {
	...
	if store, ok := asInventoryStore(s.backend); ok {
		...
		blob = data
		entries = parseInventoryBlob(data)
		rep.UnparseableLines = countUnparseableInventoryLines(data, len(entries))
	}
