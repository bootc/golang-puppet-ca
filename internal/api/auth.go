...
func sanitiseForLog(s string) string {
	if len(s) > maxLoggedValue {
		// Back off to a rune boundary. The cut is in bytes, and this is exported
		// for values that are not ASCII by construction -- a CRL's issuer DN is
		// chosen by whoever wrote the file -- so slicing at 256 can land inside
		// a multi-byte rune and emit an invalid UTF-8 sequence. strings.Map
		// below does not repair it: a decoding error yields RuneError, which is
		// not a control character, so it passes straight through into the
		// record. Trimming loses at most three bytes of a value already being
		// truncated.
		cut := maxLoggedValue
