...
var _ = Describe("Auth Middleware", func() {
	...
	Context("a certificate revoked by another replica", func() {
		...
		// The renewal endpoint is the one place where the middleware admitting a
		// stale certificate is not self-limiting, so it is worth showing the
		// whole path end to end: the middleware lets the request through on its
		// out-of-date cache, and the handler's own read-through check stops it.
		// This is the only spec where both halves are live at once — the
		// api_test.go suite runs with no AuthConfig, so its middleware is a
		// pass-through and cannot admit anything.
		It("cannot be renewed on a replica whose middleware would still admit it", func() {
			ctx := context.Background()

			csrPEM, err := testutil.GenerateCSR("peer-renew-client")
			Expect(err).NotTo(HaveOccurred())
			_, err = myCA.SaveRequest(ctx, "peer-renew-client", csrPEM)
			Expect(err).NotTo(HaveOccurred())
			certPEM, err := myCA.Sign(ctx, "peer-renew-client")
			Expect(err).NotTo(HaveOccurred())
			block, _ := pem.Decode(certPEM)
			issuedCert, err := x509.ParseCertificate(block.Bytes)
			Expect(err).NotTo(HaveOccurred())
			...
			// Precondition: the peer's cache has not caught up, so the
			// middleware's own revocation check passes this certificate.
			stale, err := peerCA.IsRevokedSerial(ctx, issuedCert.SerialNumber)
			Expect(err).NotTo(HaveOccurred())
			Expect(stale).To(BeFalse(), "precondition: the middleware would admit this certificate")

			for _, body := range [][]byte{nil, csrPEM} {
				req := httptest.NewRequest("POST", "/certificate_renewal", bytes.NewReader(body))
				req = withClientCert(req, issuedCert)
				rr := httptest.NewRecorder()
				peerMux.ServeHTTP(rr, req)
				Expect(rr.Code).To(Equal(http.StatusForbidden),
					"renewal must re-read the CRL rather than trust the middleware's verdict")
			}
		})
	})
...
//	tierPublic      allow      -                         -
//	tierOwnClient   deny       allow                     allow
//	tierSelfOrAdmin deny       allow                     deny
//	tierAdminOnly   deny       deny                      deny
//
// The probe identity is issued by this CA, so tierOwnClient's row is what
// tierAnyClient's was: the two differ only in what they do with a certificate
// from *another* trusted issuer, which this matrix does not present. That
// distinction is covered by the multi-domain specs, not here.
//
