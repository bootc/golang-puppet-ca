...
func decodeCRLChain(blob []byte) ([]*x509.RevocationList, error) {
	// Unbounded: this reads the chain this CA published, which the file path
	// below has already bounded on the way in.
	out, _, err := decodeCRLChainRest(blob, 0)
	return out, err
}
...
func decodeCRLChainStrict(blob []byte) ([]*x509.RevocationList, error) {
	// One past the cap, which is all the caller needs to refuse the file.
	out, rest, err := decodeCRLChainRest(blob, maxCRLChainFileEntries+1)
	if err != nil {
		return nil, err
	}
	// Checked before the boundary test below, and it has to be: stopping at the
	// limit leaves bytes unconsumed, which would otherwise be reported as a
	// truncated file. The count is deliberately not stated -- parsing stopped,
	// so it is not known, and "more than N" is what the operator acts on.
	if len(out) > maxCRLChainFileEntries {
		return nil, fmt.Errorf("the file holds more than the %d allowed CRLs: "+
			"a chain is one CRL per ancestor, so this is a directory concatenated "+
			"by accident, or a file appended to instead of replaced",
			maxCRLChainFileEntries)
	}
...
func decodeCRLChainRest(blob []byte, limit int) ([]*x509.RevocationList, []byte, error) {
	var out []*x509.RevocationList
	rest := blob
	for {
		// Stop once the caller has enough to reject the file. Without this the
		// entry cap bounds only the work *after* parsing, while a 4 MiB file of
		// minimal CRLs -- thousands of them, well under the byte bound -- is
		// parsed in full before being refused, with the CRL lock and c.mu held.
		// A limit of 0 means unbounded, for callers reading a blob this CA
		// wrote itself.
...
func (c *CA) crlChainLocked(ctx context.Context, ourCRL *x509.RevocationList, storedBlob []byte) ([]byte, error) {
	...
	if c.CRLChainFile != "" {
		upstream, stated, err := c.upstreamCRLs(ctx, storedBlob, publishingUpstream)
		if err != nil {
			return nil, err
		}
