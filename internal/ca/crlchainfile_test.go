...
var _ = Describe("crl_chain_file", func() {
	...
	// The property the in-lock comparison exists for, and the only one on this
	// path that a sequential spec cannot reach.
	//
	// RefreshCRLChainFile reads the published chain and compares it *inside* the
	// CRL lock rather than before taking it. Deciding outside meant every
	// replica that noticed the same file change re-signed: each compared against
	// the pre-change chain, each took the lock in turn, and each wrote -- one
	// refresh producing as many CRL numbers, exporter wake-ups and
	// inventory-free re-signs as there are replicas.
	//
	// Every other spec here calls this sequentially, so all of them pass with
	// the comparison hoisted back out. Concurrent callers against one CA are
	// enough to tell the difference: the read of the published chain happens
	// under c.mu and the CRL lock, so the first writer's result is what the rest
	// compare against, which is exactly the claim. A distributed backend is not
	// needed and would not help -- SupportsDistributedLocking is false for the
	// filesystem backend this suite uses.
	It("rewrites once when several passes notice the same change together", func() {
		trustUpstream()
		myCA.CRLChainFile = writeChainFile(upsCRL)

		const racers = 8
		var wg sync.WaitGroup
		rewrote := make(chan bool, racers)
		...
		for err := range failed {
			Expect(err).NotTo(HaveOccurred(), "a losing pass must return cleanly, not error")
		}
		wrote := 0
		for r := range rewrote {
			if r {
		...
		Expect(wrote).To(Equal(1),
			"one file change must produce one re-sign, however many passes see it")

		// And the chain is the one the single writer published, not a pile of
		// duplicates from eight of them.
		chain := crlBlocks(mustGetCRL(ctx, store))
		Expect(chain).To(HaveLen(2))
		Expect(chain[1].Issuer.CommonName).To(Equal("Upstream Root CA"))
	})
	...
	It("counts a published CRL whose signer has left the CA bundle", func() {
		...
		_, err := myCA.RefreshCRLChainFile(ctx)
		Expect(err).NotTo(HaveOccurred())
		// Exactly one, matching "counts an ancestor the file has stopped
		// listing": a rewriting pass evaluates the file twice -- once to decide,
		// then again inside the re-sign that publishes -- and only the pass that
		// publishes counts. A looser assertion would pass just as well if this
		// arm counted on both.
		Expect(myCA.CRLChainRemoved()).To(Equal(before+1),
			"a chain that shrinks unattributably must be counted once per ancestor")
	})
	...
	It("refuses a CA bundle holding no certificates at all", func() {
		...
		_, err := myCA.RefreshCRLChainFile(ctx)
		// ParseCABundle's wording: bundleCertificates delegates to it rather
		// than carrying a second parser, so this is the one message for an
		// empty bundle wherever it is read from.
		Expect(err).To(MatchError(ContainSubstring("no CERTIFICATE blocks")))
	})

	// The guard the duplicate parser did not have. bundleCertificates used to
	// skip every non-CERTIFICATE block silently, a private key included;
	// ParseCABundle refuses one outright, because this document is stored
	// world-readable and served unauthenticated at GET /certificate/ca. Not
	// reachable through storage written by this CA, which passed the same parser
	// on the way in -- so this pins the property against a future writer that
	// does not, rather than a live path.
	It("refuses a CA bundle carrying a private key", func() {
		trustUpstream()
		myCA.CRLChainFile = writeChainFile(upsCRL)
		Expect(store.SaveCACert(ctx, pem.EncodeToMemory(
			&pem.Block{Type: "PRIVATE KEY", Bytes: []byte("not really a key")}))).To(Succeed())

		_, err := myCA.RefreshCRLChainFile(ctx)
		Expect(err).To(MatchError(ContainSubstring("PRIVATE KEY")),
			"a key in the bundle must be named, not silently skipped")
	})
	...
	// The accept side of the same bound, which the refusal above cannot cover:
	// 65 fails whether the test is `> 64` or `>= 64`, so only a file of exactly
	// 64 distinguishes them. Without this, tightening the comparator would
	// refuse a legal file and no spec would notice.
	//
	// The 64 blocks are copies of one ancestor's CRL, so they deduplicate to a
	// single published entry -- the bound counts what the file holds, not what
	// survives it, which is the point: the parse and the trial verification are
	// paid before deduplication can help.
	It("accepts a file holding exactly as many CRLs as the entry bound allows", func() {
		trustUpstream()

		var exactly []byte
		for i := 0; i < 64; i++ {
			exactly = append(exactly, upsCRL...)
		}
		myCA.CRLChainFile = writeChainFile(exactly)

		_, err := myCA.RefreshCRLChainFile(ctx)
		Expect(err).NotTo(HaveOccurred(),
			"64 is allowed; only more than 64 is refused")
		Expect(crlBlocks(mustGetCRL(ctx, store))).To(HaveLen(2),
			"ours plus the one ancestor the 64 copies deduplicate to")
	})
...
var _ = Describe("crl_chain_file: size and duplicates", func() {
	...
	It("counts an ancestor the file has stopped listing", func() {
		...
		Expect(crlBlocks(mustGetCRL(ctx, store))).To(HaveLen(2))
		// One, not two: a pass that actually rewrites evaluates the file twice --
		// once to decide, and once more inside crlChainLocked when
		// reissueCRLLocked re-signs under the lock -- and the ancestor is still
		// in the published chain both times. Counting on both made one lost
		// ancestor read as 2, so this counter does not share the "per CRL per
		// evaluation" cadence of the discard and regression counters: it reports
		// a transition, so only the publishing pass counts. Pinned at its value,
		// because ">= 1" passes on exactly the double-count this rules out.
		Expect(myCA.CRLChainRemoved()).To(Equal(uint64(1)))
		Expect(myCA.CRLChainDiscarded()).To(BeZero(),
