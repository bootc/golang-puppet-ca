...
var _ = Describe("Renewal issuer gate", func() {
	...
	Describe("AutoRenew", func() {
		...
		It("refuses a certificate we issued but have revoked", func() {
			// The issuer gate alone would accept this: CheckSignatureFrom is a
			// pure signature check with no validity semantics. refuseIfRevoked
			// is the half that refuses it, and it re-reads the CRL from storage
			// first — which is why the sentinel here is ErrCertRevoked and not
			// the issuer gate's own.
	...
	Describe("a CA whose CRL cache is unavailable", func() {
		...
		It("refuses rather than assuming the certificate is unrevoked", func() {
			...
			// refuseIfRevoked's own wrap, not merely "some error": blind has no
			// CA key, so a fail-open check would run on to issueLeafLocked and
	...
	Describe("Renew", func() {
		...
		It("refuses a certificate we issued but have revoked", func() {
			...
			// the two paths gate separately, so dropping refuseIfRevoked from
			// this one would leave a revoked certificate usable as a re-key
			// credential while every other Renew spec still passed.
			Expect(myCA.Revoke(ctx, "node1.test")).To(Succeed())
