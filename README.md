...
- **Delayed supersession (opt-in):** by default a renewal revokes the certificate it replaced before it returns, so the two are never both valid. Set `superseded_cert_revoke_after_sec` and the replaced serial is recorded instead, and a background job revokes it once the window elapses — the overlap a certificate other parties are actively verifying needs in order to be replaced without a gap. The window is a deliberate weakening and is off by default; revoking a subject still retires anything of its own inside one. Safe across replicas (serialised on the shared CRL lock). See [delayed supersession](docs/configuration.md#delayed-supersession)
