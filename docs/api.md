...
| **Any client** | Any CA-signed cert with `clientAuth` EKU | `POST /certificate_renewal` — and the presented certificate must be one this CA issued and has not revoked; renewal reissues under our authority using the certificate's own subject and extensions, which is only safe for certificates we vetted at issuance |
