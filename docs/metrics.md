...
> revoke path only — a CRL that could not be read, a malformed serial, or an
> inventory read that failed while resolving the subject's serial. That last one
...
> answers its caller `409` in both cases — a
