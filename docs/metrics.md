...
| `puppetca_crl_update_failures_total` | Counter of failures to amend the CRL — a CRL that could not be read, re-signed or written, and a malformed serial (across the revoke, cleanup, reissue and refresh paths). A rising value means the CRL is not being maintained; for revocations it means a superseded certificate may still be a valid credential. It does not count a revocation that failed before reaching the CRL — a CRL lock that could not be taken, or a subject whose serial the inventory could not resolve — which is logged only, so treat the counter as a lower bound. Resets to `0` on process restart. |
