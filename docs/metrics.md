...
> warnings. Those two name the serial, and the certificate they left valid is
> the one case revoking by subject cannot reach — a replacement has already been
> issued, so `revoke --certname` would retire that instead. Retire it with
> `openvox-ca-ctl revoke --serial <hex>`; see
> [revocation by serial](api.md#revocation-by-serial).
>
