...
// buildClientCAPool assembles the pool Go sends as the TLS handshake's
// certificate_authorities hint: this CA's own certificate, then every anchor of
// every trust domain.
//
// Extracted for the same reason buildTrustDomains was -- so that what the
// handshake advertises is decided in one place a spec can call. It was inline in
// the serve command's closure, which no test reaches, and its failure is silent
// and total for the population it exists to serve: a client holding a
// certificate from a trusted foreign issuer decides what to offer from this
// list, so an anchor missing here means that client offers nothing and is
// rejected as anonymous, while attribution, revocation and admin scoping all
// stay green.
//
// The pool is deliberately broader than any single domain's authority. It is a
// hint, not a trust decision: the middleware still verifies each certificate
// against one domain's anchors alone, so putting every anchor in one pool
// advertises them without merging what they are trusted for.
//
// Anchors come from the domains already built rather than from a re-read of the
// files. Re-parsing would let the pool and the authorisation decision diverge if
// a bundle changed between the two reads -- and since it is the pool that tells
// a client which certificate to offer, the divergence would present as a client
// silently offering nothing.
//
// A CA certificate that does not parse is an error rather than an omission. The
// pool would otherwise be built without our own CA in it, and every agent we
// issued would stop offering its certificate -- the same silent, total failure,
// aimed at the whole fleet instead of one foreign issuer. In practice this is
// unreachable: CA.Init has already parsed the same bytes.
func buildClientCAPool(caCertPEM []byte, auth *api.AuthConfig) (*x509.CertPool, error) {
	block, _ := pem.Decode(caCertPEM)
	if block == nil {
		return nil, fmt.Errorf("the stored CA certificate is not PEM, so the TLS " +
			"handshake would not advertise this CA to its own agents")
	}
	ownCert, err := x509.ParseCertificate(block.Bytes)
	if err != nil {
		return nil, fmt.Errorf("parsing the stored CA certificate for the TLS handshake: %w", err)
	}

	pool := x509.NewCertPool()
	pool.AddCert(ownCert)
	if auth == nil {
		return pool, nil
	}
	for i := range auth.Domains {
		for _, anchor := range auth.Domains[i].Anchors {
			pool.AddCert(anchor)
		}
	}
	return pool, nil
