...
var _ = Describe("runCRLSync", func() {
	...
	// The sibling refresher runs a pass before its first tick; this one
	// deliberately does not, because Init has just read the same storage. The
	// distinction is only observable if the CRL advances *before* the loop
	// starts, so that is the shape this pins.
	It("does not run a pass before its first tick", func() {
		...
		Expect(c.ReissueCRL(context.Background())).To(Succeed())
		target := storedCRLNumber(store)
		behind := cachedCRLNumber(peer)
		Expect(target).To(Equal(behind+1), "precondition: the peer is one revision behind")

		go runCRLSync(ctx, peer, 750*time.Millisecond)
		...
		Consistently(func() int64 { return cachedCRLNumber(peer) }).
		...
		Eventually(func() int64 { return cachedCRLNumber(peer) }).
