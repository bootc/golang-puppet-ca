...
var _ = Describe("client_ca configuration", func() {
	...
	// buildClientCAPool: the TLS handshake's certificate_authorities hint.
	//
	// This was inline in the serve command's closure, which no spec reaches, and
	// it had stood untested through three review rounds. Its failure is silent
	// and total for the population it exists to serve: the pool is what tells a
	// client which certificate to offer, so an anchor missing from it means that
	// client offers nothing and is refused as anonymous -- while attribution,
	// revocation and admin scoping all stay green, because none of them is
	// reached.
	//
	// Asserted through x509 verification rather than through CertPool.Subjects(),
	// which is deprecated: verifying a certificate against the pool as roots asks
	// what the pool *means* to a TLS stack rather than what it holds internally.
	Describe("buildClientCAPool", func() {
		var (
			ownCA     *x509.Certificate
			serverCA  *x509.Certificate
			agentCA   *x509.Certificate
			strangerC *x509.Certificate
		)

		BeforeEach(func() {
			ownCA, _ = mintCA("Own CA", nil, nil)
			serverCA, _ = mintCA("Server CA", nil, nil)
			agentCA, _ = mintCA("Agent CA", nil, nil)
			strangerC, _ = mintCA("Stranger CA", nil, nil)
		})
		...
		// advertised reports whether a TLS stack treating pool as its root set
		// would accept this issuer -- which is exactly what the hint decides.
		advertised := func(pool *x509.CertPool, c *x509.Certificate) bool {
			GinkgoHelper()
		...
		poolFor := func(entries ...config.ClientCA) *x509.CertPool {
			GinkgoHelper()
			cfg := &serverConfig{}
			cfg.ClientCA = entries
			domains, err := buildTrustDomains(cfg, ownCA, nil)
			Expect(err).NotTo(HaveOccurred())
			pool, err := buildClientCAPool(pemOf(ownCA), &api.AuthConfig{Domains: domains})
			Expect(err).NotTo(HaveOccurred())
			return pool
		}

		It("advertises this CA and every configured anchor, and nothing else", func() {
			pool := poolFor(
				config.ClientCA{Name: "server", File: writeCertFile(serverCA)},
				config.ClientCA{Name: "agent", File: writeCertFile(agentCA)},
			)

			Expect(advertised(pool, ownCA)).To(BeTrue(),
				"our own agents decide what to offer from this list too")
			Expect(advertised(pool, serverCA)).To(BeTrue())
			Expect(advertised(pool, agentCA)).To(BeTrue(),
				"a second entry must not be lost to the first")
			Expect(advertised(pool, strangerC)).To(BeFalse(),
				"an issuer configured nowhere must not be advertised")
		})

		It("advertises this CA alone when no client_ca is configured", func() {
			pool := poolFor()

			Expect(advertised(pool, ownCA)).To(BeTrue())
			Expect(advertised(pool, serverCA)).To(BeFalse(),
				"the default deployment must advertise exactly what it always did")
		})

		It("advertises this CA when there is no AuthConfig at all", func() {
			// mTLS configured with no admin allow list and no client_ca: the
			// handshake still has to advertise us, or every agent goes anonymous.
			pool, err := buildClientCAPool(pemOf(ownCA), nil)
			Expect(err).NotTo(HaveOccurred())

			Expect(advertised(pool, ownCA)).To(BeTrue())
		})

		It("refuses a stored CA certificate it cannot parse", func() {
			// Unreachable in practice -- CA.Init has parsed the same bytes -- but
			// the alternative to failing is a pool built without our own CA in it,
			// which silently unauthenticates the entire fleet. Fail loudly instead.
			_, err := buildClientCAPool([]byte("not a certificate"), nil)
			Expect(err).To(MatchError(ContainSubstring("not PEM")))

			bad := append([]byte{}, pemOf(ownCA)...)
			_, err = buildClientCAPool(
				pem.EncodeToMemory(&pem.Block{Type: "CERTIFICATE", Bytes: bad[:20]}), nil)
			Expect(err).To(MatchError(ContainSubstring("parsing the stored CA certificate")))
		})
	})
	...
	Describe("buildTrustDomains", func() {
		...
		// warnIfGrantsSpanAnchors, completing a gate that was half covered.
		// "warns that a grant spans every anchor in a multi-anchor entry" and
		// "says nothing about spanning anchors when the entry grants nothing"
		// already pin the admin_cns arm and the no-grants arm. These two are the
		// arms neither reaches: allow_pp_cli_auth on its own, and the anchor
		// count. Each fails on its own mutation, checked.
		Describe("an entry whose grants span more than one anchor", func() {
			captureWarnings := func(cfg *serverConfig) string {
				GinkgoHelper()
			...
			It("warns on allow_pp_cli_auth alone, with no admin_cns", func() {
				// The wider of the two grants and the easier to set without thinking
				// about the bundle: it delegates admin admission to the issuer, so
				// spanning anchors delegates it to all of them. Dropping
				// AllowPpCliAuth from the gate leaves every other spec green.
				cfg := &serverConfig{}
				cfg.ClientCA = []config.ClientCA{{
					Name: "partners", File: writeCertFile(serverCA, root),
					AllowPpCliAuth: true,
				}}

				Expect(captureWarnings(cfg)).To(
					ContainSubstring("more than one anchor"))
			})

			It("stays quiet on a single anchor, however wide the grant", func() {
				// The scoping the documentation promises, and what splitting a bundle
				// per issuer produces. Without this the gate could lose its anchor
				// count and warn on every scoped entry -- the failure that trains
				// operators to ignore a warning rather than the one that hides it.
				cfg := &serverConfig{}
				cfg.ClientCA = []config.ClientCA{{
					Name: "server", File: writeCertFile(serverCA),
					AdminCNs: []string{"ops-admin"}, AllowPpCliAuth: true,
				}}

				Expect(captureWarnings(cfg)).NotTo(
					ContainSubstring("more than one anchor"))
			})
		})
...
var _ = Describe("refreshClientCRLs", func() {
	...
	// runClientCRLReloader's own loop. Everything below drives refreshClientCRLs
	// directly, so the loop that decides *when* it runs -- and when it stops --
	// was covered by nothing: delete the ticker arm and every spec here stays
	// green while a foreign issuer's CRLs are read once at startup and never
	// again, which is the failure the job exists to prevent.
	//
	// Both arms, and both asserted on observable outcome rather than on the
	// mechanism: a spec that counted ticks, or watched the ticker, could be
	// satisfied by a loop that ticks and does nothing with it.
	Describe("runClientCRLReloader", func() {
		// A fixed path both CRLs are written to in turn, so the reloader is
		// re-reading the same configured file rather than being pointed at a
		// new one -- which is what an operator's refresh mechanism does.
		writeAt := func(path string, crl *x509.RevocationList) {
			GinkgoHelper()
			Expect(os.WriteFile(path,
				pem.EncodeToMemory(&pem.Block{Type: "X509 CRL", Bytes: crl.Raw}), 0o644)).To(Succeed())
		}

		It("picks up a changed crl_file on its own tick, and stops when the context is cancelled", func() {
			path := filepath.Join(GinkgoT().TempDir(), "server-crls.pem")
			// Starts expired, so the domain has no usable revocation material
			// and the gauge reads 0. Nothing but a later tick can change that.
			writeAt(path, mintCRLUntil(serverCA, serverKey, time.Now().Add(-time.Minute)))

			cfg := &serverConfig{}
			cfg.ClientCA = []config.ClientCA{
				{Name: "server", File: writeCertFile(serverCA), CRLFile: path},
			}
			domains, err := buildTrustDomains(cfg, ownCA, nil)
			Expect(err).NotTo(HaveOccurred())

			refreshClientCRLs(cfg, domains, metrics)
			Expect(gauge("server")).To(Equal(0.0), "an expired CRL leaves the domain unusable")
			...
			go func() {
				defer GinkgoRecover()
				defer close(stopped)
				runClientCRLReloader(ctx, cfg, domains, metrics, 10*time.Millisecond)
			}()
			DeferCleanup(func() {
				cancel()
			...
			// The tick arm: the operator replaces the file, and nothing else
			// tells the process about it.
			writeAt(path, mintCRL(serverCA, serverKey))
			Eventually(func() float64 { return gauge("server") }, 2*time.Second, 10*time.Millisecond).
				Should(Equal(1.0), "the job must re-read the file on its own timer")

			// The stop arm: cancelling must return, rather than leaving a
			// goroutine reading an operator's file for the life of the process.
			cancel()
			Eventually(stopped, time.Second).Should(BeClosed(),
				"the job must return when its context is cancelled")

			// And having returned, it must stay stopped. Without this the stop
			// arm is satisfied by a loop that returns and is restarted, or by
			// one that never ran at all.
			settled := lastReload("server")
			writeAt(path, mintCRLUntil(serverCA, serverKey, time.Now().Add(-time.Minute)))
			Consistently(func() float64 { return lastReload("server") }, 200*time.Millisecond, 20*time.Millisecond).
				Should(Equal(settled), "a cancelled job must not go on applying the file")
		})
	})
