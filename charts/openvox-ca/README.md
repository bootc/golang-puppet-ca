...
| `resources` | 10m CPU / 48–64Mi | The memory limit is a hard cap and the footprint grows with the certificate count; raise it before a growing inventory reaches it. The launcher also divides this ceiling across the three server processes, claiming 90% and giving the launcher 8Mi, the signer 24Mi and the frontend the remainder — 25.6Mi at the 64Mi default. 63Mi is the smallest whole MiB that divides: **below it nothing is divided and no process gets a memory limit at all**, so lowering this value past the default disables the mechanism rather than tightening it. **Upgrading an install that leaves this at 64Mi cuts the frontend's usable heap from most of the limit to 25.6Mi**, since nothing capped it before; raise the limit at upgrade time if your fleet was near the old ceiling. See [sizing](https://github.com/voxpupuli/openvox-ca/blob/main/docs/helm-chart.md#sizing) |
